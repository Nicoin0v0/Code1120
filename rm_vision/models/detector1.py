...
class ArmorDetector:
    ...
    def process_img(self, img, val):
        ...
        img_darker = self.darker(img_dark)  # 降低亮度
    ...
    def find_light(self, resized_img, binary_img):
    ...
    def is_armor(self, img, lights, light_angle_tol=5, line_angle_tol=7, height_tol=10, width_tol=10, cy_tol=5):
        lights_matched = []
            ...
            if light_matched:
                ...
                if 200 <= armor_raw[1][0] * armor_raw[1][1] <= 11000:
                    ...
                    if 1 <= armor_flit[1][1] / armor_flit[1][0] <= 3.5:
                        armors.append(self.adjust(armor_flit))
        armors_center = []
    ...
    def detector(self, img_raw):
        resized_img, binary_img = self.process_img(img_raw, 35)#返回binary
        # 找灯条
        img_light = resized_img.copy()
        img_light, rects = self.find_light(img_light, binary_img)
        # 找装甲板
        img_armor = resized_img.copy()
        armors_center = self.is_armor(img_armor, rects)
...
if __name__ == "__main__":
    ...
    for name in img_paths:
        img = cv2.imread(name)
        armors_center = detector.detector(img)
        cv2.waitKey(0)
